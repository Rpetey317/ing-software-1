...
- **_Descriptivo:_** Qué tan bien describe la realidad, qué tan claro es de entender. El mejor código es el que es tan fácil de leer como lo fue de escribir. Como ya discutimos, un programa no es sólo instrucciones para una computadora. Cosas que resultan indistintas a la máquina (nombre de variables, comentarios, etc.) no son indistintas para los desarrolladores ni los usuarios, por lo que son tan importantes como los aspectos técnicos.
     ...
     - Utilizar _buenos nombres_ que ayuden a entender el código a nivel conceptual.
     - Corresponde a la **parte artística** del desarrollo
- **_Implementativo:_** Qué tan eficiente es el programa con los recursos. Al fin y al cabo, el fin último del software es correr en una máquina.
     - Debe _ejecutarse_ de forma correcta y con la menor cantidad de errores posible.
     - Además debemos buscar buena _performance_ del programa.
     - Una vez terminado se pueden buscar mejoras de _optimización_ de recursos.
     - Corresponde a la **parte detallista** del desarrollo
...
Recientemente, se re-planteó el enfoque que se le da al desarrollo de software. En 2001, varios expertos en el tema se reunieron para escribir el **_Manifiesto por el Desarrollo Ágil de Software_**, que pone especial énfasis en el _feedback inmediato_:
...
Este cambio de paradigma premia los ciclos de producción cortos: intercambios de a pares constantes, tests cada par de minutos minutos, reuniones cada par de días, e incluso releases cada par de meses. La idea es que, para adaptarse al mundo y los clientes cambiando constantemente, la ingeniería de software debería ser igual de flexible. Debemos aplicar los conocimientos prácticos y precisos de la ingeniería al proceso artístico que es desarrollar una aplicación.
